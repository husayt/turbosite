      ...
      <div class="py-12 md:py-20">
        ...
        <div class="max-w-3xl mx-auto text-center pb-12 md:pb-20">
          ...
          <p class="text-lg text-slate-400">
            There are many variations available, but the majority have suffered, by injected humour, or randomised words
        ...
        <div class="grid lg:grid-cols-3 gap-4 sm:gap-6 max-w-xs mx-auto lg:max-w-none">
          <template v-for="(item, index) in items" :key="index">
            <div
              class="relative p-5 before:absolute before:inset-0 before:-z-10 before:border before:border-slate-300 before:bg-slate-700 before:opacity-10 before:rounded-xl"
            >
              <div class="flex items-center justify-between space-x-2 mb-4">
                <div class="flex items-center space-x-4">
                  ...
                  <div class="grow truncate">
                    <div class="font-bold text-slate-100 truncate">
                      {{ item.name }}
                    </div>
                    <a
                      class="text-sm text-slate-500 hover:text-slate-300 font-medium truncate transition-colors"
                      :href="item.twitter.link"
                    >{{ item.twitter.handle }}</a>
                  </div>
                </div>
